...
export default function TeamActivity({ teamMetrics = [], loading = false }) {
    if (loading) {
        return (
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-6">
                ...
                <span className="badge-info">{teamMetrics.length} Members</span>
            </div>
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="relative">
                                        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-accent to-accent-light flex items-center justify-center font-bold text-lg overflow-hidden">
                                            {member.avatar ? (
                                        ...
                                        <span className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-dark-700 ${member.load < 100 ? 'bg-success' : 'bg-danger'}`} />
                                    </div>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs text-text-muted bg-dark-600 px-2 py-0.5 rounded truncate">{member.role}</span>
                                        </div>
                                    ...
                                    <div className="text-right">
                                        <p className={`text-lg font-bold ${loadColor.text}`}>{member.load || 0}%</p>
                                ...
                                <div className="mt-3 h-2 bg-dark-600 rounded-full overflow-hidden">
                                    <motion.div
                                        ...
                                        initial={{ width: 0 }}
                                        animate={{ width: `${Math.min(100, member.load || 0)}%` }}
